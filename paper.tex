...
At the first one the user deposits his funds for playing at the casino.
...
The main downside of implementing the whole game logic inside a smart contract is the slow gambling experience for the user.
...
Secure winnings payout implies it should always possible for \gls{u} to get his winnings.
In no circumstances it should be possible for the operator to withhold the winnings of \gls{u}.
The random number generation should only depend on \gls{s} and \gls{u}.
...
To start playing the user deposits his funds to their smart contract.
For random number generation a server and user generated a random number (seed) is combined.
The hash of the server seed is shown to the user before every bet.
To check the fairness the user would have to check the server seed hash and combine his seed and the server seed
...
\begin{itemize}
    \item Provable payout of the user funds and winnings. It should not be possible for the casino to withhold the user
        funds or user winnings.
    \item The bet result generation should be fast.
        It should not take more than one second until the result is available to the user.
    \item The random number generation should not be dependent on third party services.
        The resulting random number for every bet should only be dependent on both \gls{s} and \gls{u}.
        Both should not know the result beforehand.
\end{itemize}

Providing fast bet resolution, which implies the result should be available to the user under one second, the implementation
...
For every bet the current game state is countersigned by both \gls{s} and \gls{u}.
...
Creating a game session, the user funds used for placing bet are deposited in the smart contract.
\Gls{s} does not have access to them so it can not withdraw or otherwise remove the user funds.
The house stake used to pay out the winnings is also managed by the smart contract.
So, the users can check if the house funds are sufficient.

For random number generation both \gls{u} and \gls{s} generate random seeds and generate,
...
The seed of \gls{s} a \gls{u} are combined and hence the random number generated.
...
Before placing bets, the user needs to initiate a game session.
...
\Eg the function called by the user to create a games session is written as \texttt{createGame}.

\subsection{Creating a game session}\label{subsec:creatingGameSession}
For starting the game session \gls{u} generates a random value $r_p$ and uses it as a seed for generating a hash chain by hashing iteratively the
...
To start a new game session \gls{u} sends a request to \gls{s}.
...
\begin{description}
    ...
    \item[userAddress] Address of the user; formally $D_{pa}$
    \item[previousGameId] Id of the previous game session of $D_{pa}$. Initial 0; formally $D_g$
    \item[createBefore] Unix timestamp. Only before that timestamp the user is allowed to create a new game session; formally $D_t$
    \item[serverEndHash] Last hash chain entry of \gls{s}; $\text{hc}(r_s, n - 1)$
\end{description}

To finally start a new game session \gls{u} deposits his funds for the game session and publish his $\text{hc}(r_p, n - 1)$
...
\begin{description}
    ...
    \item[number] Number chosen by user. \Eg between 1 and 98 for dice; formally: $B_n$
    \item[value] Amount user wants to bet; formally: $B_v$
    ...
    \item[userHash] Current user hash; formally: $B_{ph} = \text{hc}(s_s, n - B_r)$.
...
\begin{description}
    \item[userSignature] User' s signature over bet data $B$; formally: $S_p$
    \item[serverSignature] Server' s signature over bet data $B$; formally: $S_s$
\end{description}

\subsubsection{Initiating a bet}
To place a bet the user increments the current round id, sets the game type $g$, chooses a number $n$, a value $v$, sets
the hashes of \gls{s} and \gls{u} and the balance $b$. $b$ is the balance of the previous bet state added to
the user profit of the last bet.
...
\end{align}
The new bet state $B*$ is signed by \gls{u}. %TODO How is bet signed???
The signature and $B^*$ is send to \gls{s}.
\Gls{s} verifies the signature the bet state $B^*$. %TODO How is the bet verified!!!
If verification passes \gls{s} signs the bet $B^*$ and returns the signature to \gls{u}
Subsequent \gls{u} verifies the signature of \gls{s}.

\subsubsection{Revealing the seed}
If the signature \gls{s} is valid \gls{u} sends his seed $p_s = hc(r_s, n - B^*_r - 1)$, which is
nothing more than the previous entry in the hash chain, to \gls{s}.
\Gls{s} verifies the seed. Formally:
\begin{equation}
    \vdash \kec(p_s) = B^*_{ph}
\end{equation}
If verification succeeds \gls{s} returns its seed $s_s = hc(r_p, n - B^*_r - 1)$ to \gls{u}.
\Gls{u} verifies the seed:
\begin{equation}
...
To end the current game session \gls{u} signs a special bet state with game type $B_t = 0$.
...
\gls{u} to the smart contract:
\begin{equation}
    \texttt{serverEndGame}(B^*, S_p^*)
\end{equation}
The smart contract validates the signature and sends the balance added to funds initial deposited by the user back to him.

\subsection{Conflict Resolution}\label{subsec:conflictResolution}
In the above chapter we have described how a regular game session, when user and server comply with the rules, is handled.
Know will examine which conflicts between user and server can arise and describe how
...
\Gls{u} starts game session $g$ and the server stops responding.
With $\texttt{userCancelActiveGame}(g)$ the user can initiated the termination of the game session.
After specific time $t_s$ has passed to give the server time to respond the user can force the termination
...
With the current game state $G = (B,S)$ \Gls{u} can push the current bet state $B$, his seed
$p_s=\text{hc}(p_s, n - B_r - 1)$ and the signature of \gls{s} to the smart contract:
\begin{equation}
    \texttt{userEndGameConflict}(B, p_s, S_s)
\end{equation}
\Gls{s} has time $t_s$ to push a newer bet state $B^*$ or confirm state of \gls{u}.
If the server does not respond in time $t_s$ \gls{u} can force the game session termination.
The bet state $B$ is used by the smart contract to calculate users earnings.
The user earnings added to the initial deposited funds are send back to \gls{u}. % TODO: balance is added

\subsubsection{\Gls{s} does not respond to the reveal seed request}\label{subsubsec:serverNotRevealingSeed}
\Gls{u} placed a bet $B^*$, \gls{s} signed the bet and returned its signature to \gls{u}.
In the next step \gls{u} sends his seed to \gls{s} but it does not return its seed.
\Gls{u} can now publish the current bet state $B^*$ and his seed
$p_s=\text{hc}(r_s, n - B_r - 1)$ to the smart contract:
\begin{equation}
    \texttt{userEndGameConflict}(B, p_s, S_s)
\end{equation}
If \gls{s} does not reveal its seed int time $t_s$ to the smart contract, the user can
force the termination of the game session and the bet is processed as the user has won.


\subsubsection{\Gls{u} creates a game session but does not play}\label{subsubsec:userStopsPlaying}
\Gls{s} can initiate the termination of the game session:
\begin{equation}
    \texttt{serverCancelActiveGame}()
\end{equation}
\Gls{u} has time $t_p$ to push newer bet state.
If he does not, \gls{s} can force the termination of the game session.
The funds deposited by \gls{u} are send to back to him.

\subsubsection{\Gls{u} stops playing without ending the game session}
With the game state $G = (B,S)$ \gls{s} can publish last bet state $B$, his seed $s_s=\text{hc}(r_s, n - B_r - 1)$
and the user seed $p_s=\text{hc}(r_s, n - B_r - 1)$ to the contract:
\begin{equation}
    \texttt{serverEndGameConflict}(B, s_s, p_s, S_p)
\end{equation}
If \gls{u} does not publish a newer one or confirms the current state in time $t_p$, \gls{s} can force the termination
of the game session.
The current balance added to the initial deposited funds are send back to \gls{u}.

\subsubsection{The user does not reveal his seed}
The bet $B^*$ is signed by \gls{u} and \gls{s} and in the next step \gls{u} would need to reveal his seed.
Without \gls{u} revealing his seed both \gls{u} and \gls{s} can not calculate the random number needed to get the bet result.
So, this case can be handled as the bet $B^*$ was never placed and the user stopped playing, see~\autoref{subsubsec:userStopsPlaying}.

This is the only real difference in user and server side conflict handling.
If \gls{s} does not reveal his seed the bet can bet ignored as both \gls{s} and \gls{u} do not know the bet outcome.
But the case the server does not reveal its seed special logic is needed.
As in the previous step \gls{u} send his seed to \gls{s}, so \gls{s} can calculate the result.
...
The complete game session can be verified by the user.
The payout of the winnings and funds of the user cannot be withhold by the operator.
